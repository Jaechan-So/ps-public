...
public:
  int findDistance(TreeNode *root, int p, int q) {
    map<int, pair<int, int>>
        childrens; // children node's val -> (parent node's val, height)
    childrens.insert(make_pair(
        root->val, make_pair(0, 1))); // Just set parent of root as zero
    ...
    while (!nodes.empty()) {
      ...
      if (currentNode->left != nullptr) {
        childrens.insert(
            make_pair(currentNode->left->val,
                      make_pair(currentNode->val, currentHeight + 1)));
        nodes.push(make_pair(currentNode->left, currentHeight + 1));
      }
      if (currentNode->right != nullptr) {
        childrens.insert(
            make_pair(currentNode->right->val,
                      make_pair(currentNode->val, currentHeight + 1)));
